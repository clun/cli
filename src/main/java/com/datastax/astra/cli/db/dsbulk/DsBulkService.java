...
public class DsBulkService {
    ...
    /** Operations. */
    private enum DsBulkOperations { load, unload, count };
    ...
    private List<String> addCoreOptions(List<String> options, AbstractDsbulkCmd cmd) {
        ...
        // Cloud Secure bundle
        options.add("-b");
        Database db = dbDao.getDatabase(cmd.db);
     ...
     *      current db name
     */
    private void run(List<String> commandDsbulk, String dbName) {
        if (!isInstalled()) {
            install();
        }
